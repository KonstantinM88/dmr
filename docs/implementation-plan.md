...
`payment-model.md` §6). Применена миграция
`20260818022747_stage4_billing_payments`; денежные инварианты, единственная
активная попытка и provider-idempotency дополнительно защищены индексами и
CHECK constraints PostgreSQL.

Проверки: `lint`, `typecheck`, production `build`, 252 unit-теста, 20/20
Playwright smoke в mobile/desktop профилях, QR-flow страницы оплаты,
fail-closed поведение Stripe, migration status и DB-инварианты. Полный
успешный/неуспешный Stripe test-mode сценарий и повторная доставка валидно
подписанного webhook остаются обязательной ручной проверкой после добавления
трёх тестовых Stripe-переменных и запуска Stripe CLI; live-ключи на этом
этапе запрещены.

## Этап 5 — Будущие способы оплаты (только по отдельной команде владельца)

**Статус: согласованный scope установлен и локально проверен 2026-08-18.**

Текущий согласованный scope: наличные с обязательным подтверждением
официантом, оплата всего остатка или выбранных позиций, последовательные
платежи разных гостей наличными/Stripe, полное закрытие оплаченного стола,
вызов официанта с подтверждением и временем ожидания.

Не входят в текущий scope и требуют отдельной команды: внешний терминал,
деление одной позиции/произвольной суммы, равный сплит, чаевые, возвраты и
сменная сверка.

Проверки: `lint`, `typecheck`, production `build`, 259 unit-тестов,
migration status/DB constraints и QR HTTP-smoke. Встроенный browser binding
в сессии отсутствовал; call→ack→resolve и cash confirm оставлены как
обязательная ручная проверка владельца.
