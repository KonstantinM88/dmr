...
1. На Этапе 0/1 — прямой тест SSE route handler на реальном Hostinger
   Node.js Web App деплое (не на localhost): держим соединение открытым
   N минут под сколько-то параллельных клиентов, проверяем таймауты
   прокси/буферизацию.
2. Если стабильно — используется `RealtimeTransport` с SSE-реализацией
   для guest order statuses, waiter dashboard, kitchen/bar queue.
3. Если нестабильно — polling fallback: guest-статусы каждые 5–10 c,
...
4. Reconnect и восстановление состояния — обязательны в обеих ветках.
5. Redis/внешний realtime-provider не подключается без измеренной
