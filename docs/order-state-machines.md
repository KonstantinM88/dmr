...
- `CLOSED` и `CANCELLED` серверно инвалидируют все гостевые QR-access grant,
  выданные до закрытия. Старая вкладка больше не может заказать, оплатить или
  вызвать официанта. Тот же напечатанный QR-код остаётся действующим, но для
  нового посещения его нужно отсканировать повторно; только после нового
  `/t/[token]` первый заказ создаёт новую `DiningSession` и нового участника.
