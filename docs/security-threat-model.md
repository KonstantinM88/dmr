...
| Сайт неожиданно запрашивает камеру или зацикливает запрос после отказа | `getUserMedia` вызывается только после явного действия гостя; отказ переводит UI в platform-specific справку и требует отдельного повторного нажатия. Fallback через системную камеру/file picker не держит видеопоток. Кадры и выбранные фото обрабатываются локально в браузере и не отправляются на сервер |
| Ушедший гость сохраняет старую вкладку и вмешивается в следующее посещение | `/t/[token]` выдаёт подписанный access grant с `issuedAt`; после `CLOSED/CANCELLED` сервер отклоняет все более старые grant для заказа, оплаты, WaiterCall и guest realtime. Новый визит требует повторного QR-entry, прежний participant к новой DiningSession не привязывается |
| Ушедший гость заранее сохранил сам статический QR URL и открывает его удалённо | Статический QR — bearer URL и программно не доказывает физическое присутствие. Текущая мера ограничивает случайный доступ и оставленные вкладки, а первый заказ всё равно требует официанта. Для защиты от намеренного повторного входа нужен отдельный visit PIN или подтверждение нового устройства официантом; автоматическая ротация после каждого счёта не применяется, так как сделает напечатанный QR/NFC недействительным |
...
| NFC-метка скопирована или ведёт на устаревший стол | NFC хранит тот же opaque bearer URL, что QR; URL дополнительно валидируется `/t/[token]`, скомпрометированный токен отзывается/ротируется, после чего одновременно обновляются QR и NFC. Не помещать в NDEF внутренние ID или секреты кроме предназначенного table token |
...
| Утечка realtime-данных через кэш | Все polling endpoints отвечают `private, no-store`; guest feed разрешает стол только по подписанному и не отозванному закрытием HttpOnly QR-access grant, staff feeds — только по revocable staff session |
