...
- Guest: `/[locale]` (меню, корзина, submit/status и резервный client-side
  сканер), `/t/[token]` (QR entry). Основной ресторанный сценарий — открыть
  обычную камеру телефона и перейти по распознанной ссылке: установка
  приложения и доступ сайта к камере для этого не нужны. Встроенный сканер
  запускает `getUserMedia` только после явного нажатия кнопки разрешения,
  показывает отдельные Android/iOS-инструкции при блокировке и не пытается
  обойти системное решение браузера. Дополнительный fallback открывает
  системную камеру через file capture или выбирает готовое фото QR без
  постоянного видеопотока; поддержка конкретного camera picker зависит от ОС.
  Сканер разрешает только точный
  `/t/<opaque-token>` на текущем или настроенном origin и проходит через тот
  же server-side QR/cookie flow без отдельного тестового обхода. `/t/[token]`
  заменяет bearer QR на подписанный HttpOnly access grant с временем входа.
  После `DiningSession.CLOSED/CANCELLED` все grant, выданные не позже
  `closedAt`, отклоняются во всех guest-domain actions; следующий визит требует
  нового перехода через QR, но напечатанный код остаётся тем же.
...
- Admin tables/QR: `/[locale]/admin/tische`; защищённая правом
  ...
  Components: QR PNG строится в Server Component. Печатные карточки объясняют
  гостю основной трёхшаговый сценарий через обычную камеру телефона.
- Будущие NFC-метки не требуют нового guest route или отдельной серверной
  модели: в NDEF URI записывается тот же HTTPS URL `/t/<opaque-token>`, что и
  в QR. QR остаётся обязательным визуальным fallback. После ротации токена
  печатный QR нужно заменить, а перезаписываемую NFC-метку — обновить; если
  метка была необратимо заблокирована для записи, её придётся заменить.
  Кроссплатформенный admin writer на Web NFC не планировать как основной
  путь: метки записываются телефоном с NFC/native utility либо отдельным
  USB writer.
